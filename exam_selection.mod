...
param preferable_hospital {patients, hospitals};#hospitals preferred by the patient 

# Variables
var x{i in patients, j in hospitals} binary; # indica se o paciente i é alocado no hospital j.

# Objective function
maximize Risk: sum{i in patients, j in hospitals} x[i,j] * r[i];

# Constraints
# a patient can be tested in only one hospital
subject to one_vez{i in patients}:
